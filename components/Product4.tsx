...
import { Sprout, BookOpenCheck, Clock3, ShieldCheck, History as HistoryIcon, RefreshCw, Sparkles } from 'lucide-react';
...
const Product4: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const prompt = `Tạo một đề thi trắc nghiệm môn Công nghệ lớp ${grade} phần Nông nghiệp, độ khó ${difficulty}, theo cấu trúc mới nhất của Bộ GD&ĐT năm 2025.
      Đề thi gồm 2 phần:
      ...
      Nội dung câu hỏi phải bám sát chương trình Công nghệ lớp ${grade} phần Nông nghiệp.
    ...
    if (user) {
      try {
        await api.exams.create({
          ...
          description: `Đề thi Nông nghiệp - Lớp ${grade} - ${difficulty}`,
  ...
  return (
    <ProductTemplate
      icon={<Sprout className="w-28 h-28 text-white/40" />}
      title="Sản phẩm học tập số 4: Tạo đề thi Nông nghiệp"
      subtitle="Đề thi chuẩn tốt nghiệp THPT Quốc Gia - 28 câu (24 TN + 4 Đ/S), 50 phút | Công cụ hỗ trợ học tập - Nội dung tham khảo"
      heroGradientFrom="from-emerald-600"
    ...
    >
      ...
      {questions.length === 0 && !loading && (
        <div className="glass-card p-12 text-center animate-fade-in">
          ...
          <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-3">Bắt đầu tạo đề thi Nông nghiệp</h3>
          ...
          <div className="flex justify-center gap-4 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
            <span className="text-4xl" title="Trồng trọt">🌾</span>
            <span className="text-4xl" title="Chăn nuôi">🐄</span>
            <span className="text-4xl" title="Lâm nghiệp">🌲</span>
          </div>
        </div>
      )}
